          ...
          <table>
            ...
            <tbody>
              <tr v-for="(list,key) in inoutInfo" :key="key" @click="listInfoChoice(list)">
                ...
                <td>{{ list.sdate }}</td>
                <td>{{ list.schkdate == '' ? '-' : list.schkdate }}</td>
                <td>{{ list.edate }}</td>
...
var cont=new Vue({
  ...
  data:{
    ...
    api_inout_outcheck_list: 'http://ic.sportsdiary.co.kr/api/inout_manager/inout_outcheck_list.asp',
    api_inout_update: 'http://ic.sportsdiary.co.kr/api/inout_manager/inout_outcheck_update.asp',
    // 셀렉트 박스
    date: '',  // 날짜
  ...
  methods:{
    ...
    loadInoutList:function() {
      ...
      axios.post(_this.api_inout_outcheck_list,{
        sdate: _this.date,

    ...
    <div class="l_search_box m_ibarea">
      ...
      <div class="l_photo_area">
        <div class="l_photo_box">
          ...
          <canvas class="l_photo_img" v-bind:class="{l_contain:photosize=='v', l_cover:photosize=='h'}" v-bind:style="{backgroundImage:'url('+photo+')'}"></canvas>
          <p v-if="photo=='/front/img/icon_profilephoto.svg'">480*640(jpg, png)</p>
        </div>
      ...
      <div class="l_search_field_area">
        <div class="l_search_field">
          ...
          <div class="l_field">
            ...
            <input type="text" id="birthday" placeholder="생년월일을 입력해주세요" :value="nav.addBirthDot(birthday)" v-on:input="birthday=$event.target.value.replace(/[.]/g,'');">
          </div>
        </div>
        <div class="l_search_field">
          <div class="l_field">
...
<script>
    ...
    watch:{
      ...
      listInfoChoice:function(idx){
        ...
        this.photo=this.playerlist[idx].photo;
      ...
      loadData:function(){
        ...
        // 성별 옵션 목록 불러오기
        var genderData=axios.post(_this.select_api,{code:"GD"});
        // 선수구분 옵션 목록 불러오기
        var playerData=axios.post(_this.select_api,{code:"PT"});
        // 종목 옵션 목록 불러오기
        var sportsData=axios.post(_this.select_api,{code:"SP"});
        // 소속 옵션 목록 불러오기
        var areaData=axios.post(_this.select_api,{code:"AA"});
        // 직위 옵션 목록 불러오기
        var positionData=axios.post(_this.select_api,{code:"PS"});
        // 장애여부 옵션 목록 불러오기
        var disabledStateData=axios.post(_this.select_api,{code:"DS"});
        // 장애유형 옵션 목록 불러오기
        var disabledTypeData=axios.post(_this.select_api,{code:"DT"});
        // 경기등급 옵션 목록 불러오기
        var gradeData=axios.post(_this.select_api,{code:"DG"});

        // 한 번에 불러오기
        axios.all([
          genderData, playerData, sportsData, areaData, positionData, disabledStateData, disabledTypeData, gradeData
        ]).then(axios.spread(function(resgender, resplay, ressports, resarea, resposition, resdistate, resditype, resgrade){
          if(resgender.data.state==="true"){
            _this.select_genderlist=resgender.data.gender;
          }
          if(resplay.data.state==="true"){
            _this.select_playerlist=resplay.data.player_type;
          }
          if(ressports.data.state==="true"){
            _this.select_sportslist=ressports.data.sports;
          }
          if(resarea.data.state==="true"){
            _this.select_arealist=resarea.data.area;
          }
          if(resposition.data.state==="true"){
            _this.select_positionlist=resposition.data.position;
          }
          if(resdistate.data.state==="true"){
            _this.select_disabled_statelist=resdistate.data.desabled_state;
          }
          if(resditype.data.state==="true"){
            _this.select_disabled_typelist=resditype.data.disabled_type;
          }
          if(resgrade.data.state==="true"){
            _this.select_gradelist=resgrade.data.disabled_grade;
          }
